...
fn build_spark_config() -> Result<(), Box<dyn std::error::Error>> {
    println!("cargo:rerun-if-changed=../sail-spark-connect/data/spark_config.json");
    ...
    let config =
        serde_json::from_str::<SparkConfig>(&std::fs::read_to_string("../sail-spark-connect/data/spark_config.json")?)?;
