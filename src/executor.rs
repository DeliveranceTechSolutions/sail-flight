...
impl Executor {
    ...
    async fn run_internal(
        ...
        // For now, just use the first schema type or create a default one
        let schema_type = schema.first().cloned().unwrap_or_else(|| DataType {
            kind: Some(data_type::Kind::String(data_type::String {
                type_variation_reference: 0,
                collation: "".to_string(),
            })),
        });
        let out = ExecutorOutput::new(ExecutorBatch::Schema(schema_type));
